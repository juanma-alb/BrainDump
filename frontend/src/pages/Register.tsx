    ...
    <div className="relative bg-white/80 dark:bg-slate-900/80 backdrop-blur-3xl border border-white/50 dark:border-slate-700/50 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.15)] rounded-[2.5rem] p-8 md:p-10 animate-in slide-in-from-bottom-8 fade-in duration-500">
        ...
        <div className="space-y-4">
          ...
          <div>
            <input
              ...
              type="text"
              {...register('username')}
              className={`w-full px-5 py-4 bg-gray-50/50 dark:bg-slate-800/50 dark:text-white border-2 rounded-2xl focus:outline-none transition-all duration-200 placeholder-gray-400 dark:placeholder-gray-500 ${errors.username ? 'border-red-500 focus:border-red-500 bg-red-50/50' : 'border-transparent focus:bg-white dark:focus:bg-slate-800 focus:border-blue-500'}`}
              placeholder="Nombre de usuario"
            />
            {errors.username && <p className="mt-1.5 text-sm text-red-600 dark:text-red-400 font-medium px-2">{errors.username.message}</p>}
          </div>
          ...
          <div>
            ...
            />
            {errors.password && <p className="mt-1.5 text-sm text-red-600 dark:text-red-400 font-medium px-2">{errors.password.message}</p>}
          </div>
        </div>
      ...
      <div className="mt-8 pt-6 border-t border-gray-100 dark:border-slate-800 text-center">
        <p className="text-m text-gray-500 dark:text-gray-400">
          ...
          <Link to="/" className="text-black-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-bold transition-colors">
            Inicia sesión
          </Link>
        </p>
      </div>
