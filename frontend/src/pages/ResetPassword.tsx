...
function ResetPassword() {
  ...
  // Nuevos estados para la UX Premium
  ...
  const onSubmit = async (data: ResetPasswordFormValues) => {
    ...
    // Validación manual de la confirmación
        ...
        <div className="relative bg-white/80 dark:bg-slate-900/80 backdrop-blur-3xl border border-white/50 dark:border-slate-700/50 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.15)] rounded-[2.5rem] p-8 md:p-10 animate-in slide-in-from-bottom-8 fade-in duration-500">
          ...
          {(!id || !token) ? (
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <input
                      ...
                      className={`w-full px-5 py-4 bg-gray-50/50 dark:bg-slate-800/50 dark:text-white border-2 rounded-2xl focus:outline-none transition-all duration-200 placeholder-gray-400 dark:placeholder-gray-500 ${
                        errors.newPassword ? 'border-red-500 focus:border-red-500 bg-red-50/50' : 'border-transparent focus:bg-white dark:focus:bg-slate-800 focus:border-blue-500'
                      }`}
                      placeholder="Nueva contraseña"
                    />
                    {errors.newPassword && (
                      <p className="mt-1.5 text-sm text-red-600 dark:text-red-400 font-medium px-2">{errors.newPassword.message}</p>
                    )}
                  </div>
                  ...
                  <div>
                    <input
                      ...
                      className={`w-full px-5 py-4 bg-gray-50/50 dark:bg-slate-800/50 dark:text-white border-2 rounded-2xl focus:outline-none transition-all duration-200 placeholder-gray-400 dark:placeholder-gray-500 ${
                        confirmError ? 'border-red-500 focus:border-red-500 bg-red-50/50' : 'border-transparent focus:bg-white dark:focus:bg-slate-800 focus:border-blue-500'
                      }`}
                      placeholder="Confirmar contraseña"
                    />
                    {confirmError && (
                      <p className="mt-1.5 text-sm text-red-600 dark:text-red-400 font-medium px-2">{confirmError}</p>
                    )}
                  </div>
                </div>
            ...
            <div className="text-center py-4 animate-in zoom-in-95 duration-500">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-8 px-2">
                Tu contraseña ha sido actualizada exitosamente. Ya puedes iniciar sesión de forma segura.
              </p>
